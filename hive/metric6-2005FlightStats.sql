...
LOAD DATA LOCAL INPATH '../data/2005-sample.csv' into table flights;
...
select tmp_table.fc, count(tmp_table.totalDelay > 0), sum(tmp_table.totalDelay), sum(tmp_table.cancelled), sum(diverted), sum(weatherCancels) from (
	select concat(yearNum,"-", padInt(monthNum), "-", padInt(dayOfMonth)) fc, nonNegatives(depDelay + arrDelay) totalDelay, cancelled, diverted, cancelByWeather(cancellationCode) weatherCancels, (depDelay + arrDelay > 0) wasDelayed from flights order by fc
